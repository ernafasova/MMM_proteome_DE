...
colnames(phos_M)
colnames(phos_M) <- paste0("S", 1:116)

class(phos_M)
usethis::use_data(phos_M, overwrite = T)

setequal(colnames(phos_M), metadata_M$new_sample_id) # regardless of order
all(colnames(phos_M) == metadata_M$new_sample_id)  # checks the order
```
# STY
```{r}
...
# valid values
```{r}
valid_counts <- colSums(!is.na(phos_M))
...
scale_fill_manual(values = c("TRUE" = "lightgreen", "FALSE" = "lightblue")) +
  ...
  labs(
    title = "Number of Phosphosites (male)",
    x = "Samples (S1-S116)",
    y = "Valid Values") +
  theme_minimal(base_size = 14) +
  theme(plot.title   = element_text(face = "bold", hjust = 0.5, size = 15),
...
ggsave(file.path("doc", "valid_values_phos_male.png"), plot = valid_values_phos, width = 10, height = 4, dpi = 300, bg = "white")
```
