...
load(here::here("data/phos_M_renamed2_longterm.rda"))
load(here::here("data/metadata_M_longterm.rda"))
load(here::here("data/phos_M_renamed2_longterm_filt_norm_batchcorr.rda"))
load(here::here("data/phos_M_renamed2_longterm_filt_norm.rda"))
```

# VV1
```{r}
# valid values
valid_counts <- colSums(!is.na(phos_M_renamed2_longterm))
...
vv1 <- ggplot(count_data, aes(x = column, y = count)) +
  ...
  labs(
    title = "Phosphosites per sample, male: longterm study (n=52)",
    x = "",
    y = "Valid Values") +
...
png(filename = "doc/boxplot_phos_male_longterm.png",
    width    = 10,
    height   = 6,
    units    = "in",
    res      = 300)
...
boxplot(phos_M_renamed2_longterm, 
        main = "Male longterm study (n=52), phospho",
        ylab = "Log2 Intensity",
        col = "lightblue")
...
df_long <- tidyr::pivot_longer(phos_M_renamed2_longterm, 
                               cols = everything(), 
                               names_to = "sample_id", 
                               values_to = "intensity")
...
density_plot <- ggplot(df_long, aes(x = intensity, color = sample_id)) +
  geom_density() +
  labs(title = "Density Distribution, male:longterm (phospho)", x = "Intensity", y = "Density") +
  theme_minimal(base_size = 14) +
  theme(
...
density_plot
ggsave(file.path("doc", "densityplot_M_phospho_longterm.png"),
...
phos_M_renamed2_longterm_filt100 <- PhosR::selectOverallPercent(phos_M_renamed2_longterm, 1.0)   #3832
phos_M_renamed2_longterm_filt70  <- PhosR::selectOverallPercent(phos_M_renamed2_longterm, 0.70)  #10031
phos_M_renamed2_longterm_filt50  <- PhosR::selectOverallPercent(phos_M_renamed2_longterm, 0.50)  #13952

phos_M_renamed2_longterm_filt <- phos_M_renamed2_longterm_filt50

usethis::use_data(phos_M_renamed2_longterm_filt, overwrite = TRUE)
```
# VV2
```{r}
# valid values
valid_counts <- colSums(!is.na(phos_M_renamed2_longterm_filt))
count_data <- data.frame(column = names(valid_counts), count = valid_counts)
count_data$column <- factor(count_data$column, levels = count_data$column)
...
vv2 <- ggplot(count_data, aes(x = column, y = count)) +
  ...
  scale_y_continuous(expand = expansion(mult = c(0, 0.1)), limits = c(0, y_limit)) +
  labs(
    title = "Phosphosites per sample, male-longterm (n=52)",
    x = "",
    y = "Valid Values"
  ) +
  theme_minimal(base_size = 14) +
...
print(vv2)
ggsave(file.path("doc", "vv2_male_phospho_longterm.png"),
...
phos_M_renamed2_longterm_filt_norm <- medianScaling(
  phos_M_renamed2_longterm_filt[1:ncol(phos_M_renamed2_longterm_filt)],
  scale = TRUE
)
phos_M_renamed2_longterm_filt_norm <- as.data.frame(phos_M_renamed2_longterm_filt)

usethis::use_data(phos_M_renamed2_longterm_filt_norm, overwrite = TRUE)

```
# Distribution after normalization
```{r}
# 1. Draw the boxplot as before
boxplot(phos_M_renamed2_longterm_filt_norm, 
        main = "Male longterm (n=52), phospho",
        ylab = "Log2 Intensity",
        col = "lightblue")

# Now, create the plot and save it
png(filename = "doc/boxplot_phos_male_renamed_longterm_filt_norm.png",
    width    = 10,
    height   = 6,
    units    = "in",
    res      = 300)
...
####
# Convert data to long format
df_long <- tidyr::pivot_longer(phos_M_renamed2_longterm_filt_norm, 
                               cols = everything(), 
                               names_to = "sample_id", 
                               values_to = "intensity")
...
density_plot_global <- ggplot(df_long, aes(x = intensity, color = sample_id)) +
  geom_density() +
  labs(title = "Density Distribution, male:longterm (phospho), norm", x = "Intensity", y = "Density") +
  theme_minimal(base_size = 14) +
  theme(
      plot.title = element_text(hjust = 0.5, face = "bold", size = 13),
      legend.position = "none")
...
density_plot_global
ggsave(file.path("doc", "densityplot_phospho_M_longterm_norm.png"),
...
# PCA3
```{r}
n_original <- nrow(phos_M_renamed2_longterm_filt_norm) 
df_nona <- na.omit(phos_M_renamed2_longterm_filt_norm) 
n_nona <- nrow(df_nona)
...
pca_day_norm <- plot_pca(
    ...
    shape_var = "strain",
    palette_name = "Set1",
    plot_title = "phospho, M, longterm (n=52)",
    ellipse = TRUE
)
...
ggsave(file.path("doc", "pca_day_norm_Mlong_norm.png"),     plot = pca_day_norm,     width = 7, height = 5, dpi = 300, bg = "white")
ggsave(file.path("doc", "pca_beatbox_norm_Mlong_norm.png"), plot = pca_beatbox_norm, width = 7, height = 5, dpi = 300, bg = "white")
ggsave(file.path("doc", "pca_strain_norm_Mlong_norm.png"),  plot = pca_strain_norm,  width = 7, height = 5, dpi = 300, bg = "white")
ggsave(file.path("doc", "pca_weeks_norm_Mlong_norm.png"),  plot = pca_weeks_norm,  width = 7, height = 5, dpi = 300, bg = "white")
ggsave(file.path("doc", "pca_strain23_norm_Mlong_norm.png"), plot = pca_strain_norm_pc23, width = 7, height = 5, dpi = 300, bg = "white")
ggsave(file.path("doc", "pca_strain13_norm_Mlong_norm.png"), plot = pca_strain_norm_pc13, width = 7, height = 5, dpi = 300, bg = "white")
...
```{r}
# Batchcorr
setequal(colnames(phos_M_renamed2_longterm_filt_norm), metadata_M_longterm$new_sample_id) # TRUE
all(colnames(phos_M_renamed2_longterm_filt_norm) == metadata_M_longterm$new_sample_id)    # TRUE

phos_M_renamed2_longterm_filt_norm_batchcorr <- limma::removeBatchEffect(
  phos_M_renamed2_longterm_filt_norm,
  batch = metadata_M_longterm$beatbox_batch
)
phos_M_renamed2_longterm_filt_norm_batchcorr <- as.data.frame(phos_M_renamed2_longterm_filt_norm_batchcorr)

usethis::use_data(phos_M_renamed2_longterm_filt_norm_batchcorr, overwrite = TRUE)

```

# PCA4 after batchcorr
```{r}
n_original <- nrow(phos_M_renamed2_longterm_filt_norm_batchcorr) # 7565
df_nona <- na.omit(phos_M_renamed2_longterm_filt_norm_batchcorr) # 6287
n_nona <- nrow(df_nona)
...
fit1_batchcorr <- lmFit(phos_M_renamed2_longterm_filt_norm_batchcorr, design_batchcorr)
...
limma_results_M_longterm_main_w36_w48   <- get_limma_results(fit2_batchcorr, "Ntac_w36_w48",   "M Main effect w36-48") # Total: 133 - UP: 84 | DOWN: 49
limma_results_M_longterm_main_w24_w48   <- get_limma_results(fit2_batchcorr, "Ntac_w24_w48",   "M Main effect w24-48") # Total: 48 - UP: 30 | DOWN: 18
...
# Diet histogram
hist(limma_results_M_longterm_J_w24_w36_batchcorr$top_table$P.Value,   main = "Male J w24-36",   xlab = "P-value", col = "darkslategray4", breaks = 30)
hist(limma_results_M_longterm_Ntac_w24_w36_batchcorr$top_table$P.Value, main = "Male Ntac w24-36", xlab = "P-value", col = "darkslategray4", breaks = 30)

hist(limma_results_M_longterm_J_w36_w48_batchcorr$top_table$P.Value, main = "Male J w36-48", xlab = "P-value", col = "darkslategray4", breaks = 30)
hist(limma_results_M_longterm_Ntac_w36_w48_batchcorr$top_table$P.Value, main = "Male Ntac w36-48", xlab = "P-value", col = "darkslategray4", breaks = 30)
```
...
fit1 <- lmFit(phos_M_renamed2_longterm_filt_norm, design)
fit2 <- contrasts.fit(fit1, contrasts)
fit2 <- eBayes(fit2)

colnames(fit2)

limma_results_M_longterm_main_w24_w36   <- get_limma_results(fit2, "main_w24_w36",   "Male Main effect w24-36") # Total: 23 - UP: 18 | DOWN: 5
limma_results_M_longterm_main_w36_w48   <- get_limma_results(fit2, "Ntac_w36_w48",   "Male Main effect w36-48") # Total: 19 - UP: 8 | DOWN: 11
limma_results_M_longterm_main_w24_w48   <- get_limma_results(fit2, "Ntac_w24_w48",   "M Main effect w24-48") # Total: 40 - UP: 25 | DOWN: 15
